...
router.post('/register', async (req, res) => {
  try {
    const { first_name, last_name, email, username, phone, password, role } = req.body;
    ...
    if (username) {
      ...
      if (existingUsername) return res.status(400).json({ msg: 'Username sudah dipakai' });
    }

    // Cek phone duplikat
    if (phone) {
      const existingPhone = await User.findOne({ phone });
      if (existingPhone) return res.status(400).json({ msg: 'Nomor HP sudah terdaftar' });
    }
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!user.is_active) return res.status(400).json({ msg: 'Akun dinonaktifkan' });
