...
router.post('/', auth, async (req, res) => {
  try {
    const { donation_id, quantity_claimed, pickup_scheduled_at, notes } = req.body;
    ...
    if (quantity_claimed > donation.quantity_remaining) {
      return res.status(400).json({ msg: `Stok tersisa hanya ${donation.quantity_remaining} ${donation.quantity_unit}` });
    }
    ...
    if (existing) return res.status(400).json({ msg: 'Kamu sudah mengklaim donasi ini' });
...
router.get('/donation/:donationId', auth, async (req, res) => {
  try {
    ...
    const claims = await Claim.find({ donation_id: req.params.donationId })
      .populate('seeker_id', 'first_name last_name username trust_score avatar_url')
      .sort({ created_at: -1 });
    res.json(claims);
  } catch (err) {
    res.status(500).json({ msg: 'Server error' });
...
router.put('/:id/confirm', auth, async (req, res) => {
  try {
    ...
    claim.tracking_log.push({ previous_status: 'pending', new_status: 'confirmed', changed_by: req.user.id });
    await claim.save();
...
router.put('/:id/pickup', auth, async (req, res) => {
  try {
    ...
    claim.tracking_log.push({ previous_status: 'confirmed', new_status: 'picked_up', changed_by: req.user.id });
    await claim.save();
...
router.put('/:id/complete', auth, async (req, res) => {
  try {
    ...
    claim.tracking_log.push({ previous_status: 'picked_up', new_status: 'completed', changed_by: req.user.id });
    await claim.save();
    ...
    if (activeClaims === 0) {
      await Donation.findByIdAndUpdate(claim.donation_id._id, { status: 'completed' });
    }
...
router.put('/:id/cancel', auth, async (req, res) => {
  try {
    ...
    if (!isSeeker && !isProvider) return res.status(403).json({ msg: 'Akses ditolak' });
    ...
    claim.tracking_log.push({ previous_status: prevStatus, new_status: 'cancelled', changed_by: req.user.id });
    await claim.save();
    ...
    if (donation.quantity_remaining > 0 && donation.status !== 'available') {
      donation.status = donation.quantity_remaining >= donation.quantity ? 'available' : 'partially_claimed';
    }
    await donation.save();

    // Notifikasi
    const notifUserId = isSeeker ? claim.donation_id.provider_id : claim.seeker_id;
    await Notification.create({
      user_id: notifUserId,
