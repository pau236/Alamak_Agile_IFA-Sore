...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    if (!conversation) return res.status(404).json({ msg: 'Conversation tidak ditemukan' });
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    await conversation.populate('provider_id', 'first_name last_name avatar_url');
...
router.post('/:id/messages', auth, async (req, res) => {
  try {
    ...
    if (!isProvider && !isSeeker) return res.status(403).json({ msg: 'Akses ditolak' });
    ...
    const receiverId = isProvider ? conversation.seeker_id : conversation.provider_id;
