...
router.get('/user/history', auth, async (req, res) => {
  try {
    const provided = await Donation.find({ provider_id: req.user.id, deleted_at: null })
...
router.get('/:id', async (req, res) => {
  try {
    const donation = await Donation.findById(req.params.id)
      .populate('provider_id', 'first_name last_name trust_score avatar_url city phone')
      .populate('category_id', 'name icon_emoji color_hex');
...
router.post('/', auth, (req, res, next) => {
  ...
  try {
    const {
      title, description, category_id, quantity, quantity_unit,
      pickup_address, pickup_city, pickup_notes,
      pickup_start_time, pickup_end_time,
      longitude, latitude, expired_at,
      is_halal, allergen_notes
    } = req.body;

    const category = await FoodCategory.findById(category_id);
    if (!category) return res.status(400).json({ msg: 'Kategori tidak valid' });

    const photos = req.files ? req.files.map((f, i) => ({
    ...
    const donation = new Donation({
      ...
      expired_at,
      is_halal: is_halal === 'true' ? true : is_halal === 'false' ? false : null,
      allergen_notes,
      photos,
    });
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    const fields = ['title', 'description', 'quantity_unit', 'pickup_address',
      'pickup_city', 'pickup_notes', 'pickup_start_time', 'pickup_end_time',
      'expired_at', 'allergen_notes'];
    fields.forEach(f => { if (req.body[f] !== undefined) donation[f] = req.body[f]; });
